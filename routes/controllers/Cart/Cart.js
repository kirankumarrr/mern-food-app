  ...
  if (!errors.isEmpty()) {
    ...
    try {
      if (req.user.role === 'owner') {
        return next(new ErrorResponse(`Owner cannot add food to cart`, 403));
      }
      ...
      if (!foundFoodItem) {
        ...
        if (!addToCart) {
          return next(new ErrorResponse(`Failed to add food to cart`, 403));
        }
        const createFoodItem = addToCart.cart.items.filter(
          (record) =>
            record.foodId._id.toString() === foundFoodItem._id.toString()
        );
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (!foundFoodItem) {
        ...
        const addToCart = await req.user.removeFromCart(foundFoodItem._id);
        if (!addToCart) {
          return next(
            new ErrorResponse(`Failed to delete item from cart`, 403)
          );
        }
        res.status(200).json({
          success: true,
      ...
      console.log('err :', err);
      return next(new ErrorResponse(`Failed to deleted food from cart`, 502));
    }
  }
});
...
exports.clearCompleteCart = asyncHandler(async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (!isCartCleared) {
        return next(new ErrorResponse(`Failed to clear items from cart`, 403));
      }
      res.status(200).json({
        success: true,
      ...
      console.log('err :', err);
      return next(new ErrorResponse(`Failed to clear items from cart`, 502));
    }
  }
});
