...
function Form(props) {
  ...
  const handleChange = (event, element) => {
    ...
    if (element.regex) {
      ...
      if (event.target.value.match(regPattern) || event.target.value === '') {
        ...
        return (
          <div key={index}>
            <TextField
              ...
              label={element.label}
              helperText={
                !!formErrors[element.field] ? element.errorMessage : ''
              }
              variant="outlined"
              value={formValues[element.field] || ''}
              onChange={(e) => handleChange(e, element)}
      ...
      <div className={classes.buttonDiv}>
        <Button
          ...
          disabled={
            ...
            Object.keys(formValues).length <= props.formList.length - 1
          }
        >
          Save
        </Button>
